...
Place this file in the folder with:
 ...
 - test.csv (used to align founder_id)
 - submission CSVs (any number). They can be:
    ...
    * label-only submissions with a column named 'retention_status' (or TARGET_COL below).

Output: submission_soup.csv (default) in the working folder.

Examples:
  python soup_submissions.py
  python soup_submissions.py --files s1.csv s2.csv --weights 1 0.5
  python soup_submissions.py --files /path/to/s1.csv /path/to/s2.csv --weights 0.6 0.4 --out final.csv
"""
...
# default data paths (use these if files are placed in working directory or if you uploaded them)
TRAIN_FILE = Path("/mnt/data/train.csv")
TEST_FILE  = Path("/mnt/data/test.csv")
SAMPLE_FILE = Path("/mnt/data/sample_submission.csv")
...
def find_submission_files():
    ...
    for p in files:
        ...
        if name in {TRAIN_FILE.name.lower(), TEST_FILE.name.lower(), SAMPLE_FILE.name.lower(), Path(__file__).name.lower()}:
            continue
        # skip obvious intermediate files you might not want (optional)
        if "oof" in name or "probas" in name or "models" in name:
            continue
        candidates.append(p)
    return candidates
...
def read_submission_as_proba(path, classes, test_df, le):
    """
    Return (founder_ids_index, prob_matrix) where prob_matrix shape = (n_test, n_classes).
    If CSV has class columns, use them (matching class names).
    If CSV has label column, convert to one-hot using classes order.
    """
    df = pd.read_csv(path)
    # ensure founder id present, else assume same order as test
        ...
        if TARGET_COL in df.columns:
            ...
            for i, ix in enumerate(idxs):
                if ix >= 0:
                    ...
                    # unknown label -> uniform distribution (safe fallback)
...
def main(argv):
    ...
    parser.add_argument("--files", nargs="+", help="List of submission CSV files to blend. If omitted, auto-discovers CSVs in current folder.")
    ...
    if not TRAIN_FILE.exists() or not TEST_FILE.exists():
        print(f"Default train/test not found at {TRAIN_FILE} and {TEST_FILE}. Looking in working directory for train/test.")
        # fallback: look in working dir
        if not Path("train.csv").exists() or not Path("test.csv").exists():
            raise SystemExit("train.csv and test.csv must exist in the working folder (or at /mnt/data/).")
        else:
    ...
    le, classes = load_classes_from_train(train_path)
    ...
    for f in files:
        try:
            ...
            continue
        # align to test_df order by founder_id
